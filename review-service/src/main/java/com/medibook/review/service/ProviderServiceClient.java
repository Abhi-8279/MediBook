...
import com.medibook.review.exception.ExternalServiceException;
...
public class ProviderServiceClient implements ProviderServiceGateway {
    ...
    @Override
    public void updateRating(String providerId, BigDecimal avgRating, long reviewCount) {
        HttpEntity<ProviderRatingUpdateRequest> entity = new HttpEntity<>(
                new ProviderRatingUpdateRequest(avgRating, Math.toIntExact(reviewCount)),
                createInternalHeaders());
        try {
            restTemplate.exchange(
                    providerServiceBaseUrl() + "/api/v1/providers/internal/" + providerId + "/rating",
                    HttpMethod.PUT,
                    entity,
                    Void.class);
        } catch (HttpStatusCodeException exception) {
            ...
            throw new ExternalServiceException("Provider service failed while updating provider rating", exception);
        } catch (ResourceAccessException exception) {
            throw new ExternalServiceException("Provider service is currently unreachable", exception);
        }
    }
    ...
    private record ProviderRatingUpdateRequest(BigDecimal avgRating, Integer reviewCount) {
    }
}
